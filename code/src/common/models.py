...
class NoSplit(nn.Module):
    def __init__(self, type="PointNet", input_dims=3):
        ...
        if type == "PointNet":
            self.pn_extractor = PointNet(input_dims=input_dims)
        elif type == "PointNet++":
            self.pn_extractor = PointNet2(input_dims=input_dims)
        elif type == "GNN":
            self.pn_extractor = GNN(input_dims=input_dims)
        self.pn_predictor = PoseNet(input_dims=1024,output_dims=6)

    def forward(self, x):
        feat_pn = self.pn_extractor(x)
        pred_pn = self.pn_predictor(feat_pn)
        return pred_pn[:,:3], feat_pn, pred_pn[:,3:], feat_pn
