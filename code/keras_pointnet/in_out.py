...
'''
def load_x_y(list_path, n_points = 1024, n_samples = -1): # -1 misses the last one!
    x = []
    y = []
    with open(list_path) as f:
        pc_files = f.read().splitlines()[:n_samples]
    ...
    def generateXY(self,mode='p'):
        while True:
            ...
            for i in range(self.n_samples // self.batch_size):
                ...
                if mode == 'p':
                    yield np.array(batch_x), np.array(batch_y_center)
                if mode == 'n':
                    yield np.array(batch_x), np.array(batch_y_norm)
                else:
